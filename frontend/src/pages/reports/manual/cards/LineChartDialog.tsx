...
const LineChartDialog: React.FC<LineChartProps> = ({ showEditButton }) => {
  ...
  useEffect(() => {
    ...
    const fetchLayout = async () => {
      try {
        ...
        const LineChartWidget = response.data.layout.widgets.find(
          (widget) => widget.widgetName === "Line Chart"
        );

        if (LineChartWidget) {
          setWidgetID(LineChartWidget.widgetId);

          if (LineChartWidget.data) {
            setTitle(LineChartWidget.data.title || "");
            setRtu(
              Array.isArray(LineChartWidget.data.rtu)
                ? LineChartWidget.data.rtu
                : []
            );
            setShowMin(!!LineChartWidget.data.showMin);
            setShowMax(!!LineChartWidget.data.showMax);
            if (Array.isArray(LineChartWidget.data.sensorData)) {
              setSensorData(LineChartWidget.data.sensorData);
            }
          }

          setDataInitialized(true);
        } else {
          console.error("Line Chart widget not found.");
        }
      } catch (error) {
      ...
      {widgetData && (
        <CardContent>
          <Typography gutterBottom sx={{ color: "#1e3a8a", fontSize: 16 }}>
            <br />
            You're in Edit Mode of Line Chart
          </Typography>
        </CardContent>
      )}
      {showEditButton && (
                  ...
                  <TableRow key={sensor.id}>
                    ...
                    <TableCell>
                      <Select
                        ...
                        sx={{ width: "100%", backgroundColor: "#FAFBFF" }}
                        error={sensor.sensorType && !sensor.sensorId}
                      >
                        <MenuItem value="" disabled>
                          Select Sensor ID
                        </MenuItem>
            ...
            <Box sx={{ marginTop: 3, display: "flex", gap: 2 }}>
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker
                  ...
                  sx={{ width: 500, backgroundColor: "#FAFBFF" }}
                />
                <DatePicker
                  label="End Date"
                  value={endDate}
